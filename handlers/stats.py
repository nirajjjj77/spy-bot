...
from utils.helpers import is_admin, espace_markdown
...
def show_stats(update: Update, context: CallbackContext):
    ...
    stats_text = f"""📊 *{espace_markdown(stats['name'])}'s Stats:*
...
def show_leaderboard(update: Update, context: CallbackContext):
    ...
    for i, stat in enumerate(top_spies, 1):
        leaderboard += f"{i}. {espace_markdown(stat['name'])}: {stat['spy_wins']} wins ({stat['spy_win_rate']:.1f}%)\n"
    
    leaderboard += "\n👥 *Top Civilians (Win Rate):*\n"
    for i, stat in enumerate(top_civilians, 1):
        leaderboard += f"{i}. {espace_markdown(stat['name'])}: {stat['civilian_win_rate']:.1f}% ({stat['civilian_wins']}/{stat['civilian_games']})\n"
    
    leaderboard += "\n🎮 *Most Active Players:*\n"
    for i, stat in enumerate(most_active, 1):
        leaderboard += f"{i}. {espace_markdown(stat['name'])}: {stat['games_played']} games\n"
